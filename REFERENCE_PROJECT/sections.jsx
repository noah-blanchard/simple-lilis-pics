...
const Hero = () => {
  return (
    <section data-screen-label="01 Hero" className="relative min-h-[100svh] w-full overflow-hidden">
      {/* Background image */}
      ...
      <header className="relative z-20 flex items-center justify-between px-6 md:px-12 pt-8 anim-fade-down">
        <a href="#" className="text-[22px] tracking-tight italic font-semibold">
          <span>Lilis</span><span className="text-accent">.</span><span>Pics</span>
        </a>
      ...
      <div className="relative z-10 px-6 md:px-12 flex flex-col items-center justify-center min-h-[calc(100svh-100px)] text-center">
        ...
        <p className="mt-8 text-white/75 text-[15px] md:text-[17px] max-w-[640px] leading-relaxed anim-fade-up d3">
          Documentary, portrait and editorial photography for brands and humans who
          want their stories told honestly — with mood, patience, and a careful eye for light.
        </p>
...
/* ─────────────────────── MARQUEE STRIP ─────────────────────── */
const Marquee = () => {
  const items = ["Editorial", "Portrait", "Documentary", "Street", "Brand", "Wedding", "Travel", "Studio"];
  const all = [...items, ...items, ...items, ...items];
  return (
    <div className="border-y border-line/60 py-6 overflow-hidden">
      <div className="marquee-track">
        {all.map((t, i) => (
          <span key={i} className="display text-3xl md:text-5xl text-white/15 uppercase tracking-tight flex items-center gap-12">
            <span>{t}</span>
...
const About = () => {
  return (
    <section data-screen-label="02 About" className="px-6 md:px-12 py-24 md:py-32">
      <SectionHeader tag="ABOUT US" lines={["Frames that speak", "louder than words."]} />
      <div className="mt-16 grid grid-cols-1 md:grid-cols-12 gap-6 items-stretch">
        ...
        <Reveal delay={0.2} className="md:col-span-4 flex flex-col justify-end">
          <p className="text-white/75 leading-relaxed text-[15px] md:text-[16px] max-w-[420px]">
            I&apos;ve been chasing light for twelve years — through quiet apartments,
            packed studios, foreign streets at dawn. The work I love most sits between
            stillness and feeling. Whether it&apos;s your face, your brand or your
            ordinary tuesday, I want it to look like itself, only honest.
          </p>
          <a href="#" className="mt-8 inline-flex items-center gap-2 text-white hover:text-accent transition-colors">
...
/* ─────────────────────── SPECIALTY GRID ─────────────────────── */
const SPECIALTIES = [
  { id:"landscape", title:"Landscape", desc:"Showcasing nature, scenery and outdoor environments." },
  { id:"street",    title:"Street",    desc:"Candid shots of everyday life and urban scenes." },
  { id:"product",   title:"Product",   desc:"Commercial product work for advertisements and e-commerce." },
  { id:"portrait",  title:"Portrait",  desc:"People's emotions, expressions and personal stories." },
  { id:"fashion",   title:"Fashion",   desc:"Editorial concepts shot for magazines and brand books." },
  { id:"macro",     title:"Macro",     desc:"Close-up work revealing detail too small to notice." },
  { id:"event",     title:"Event",     desc:"Live coverage that captures real moments as they happen." },
  { id:"wildlife",  title:"Wildlife",  desc:"Patient documentary work in natural habitats." },
];

const Specialty = () => {
  const [active, setActive] = useState("street");
  return (
    <section data-screen-label="03 Specialty" className="px-6 md:px-12 py-24 md:py-32">
      <SectionHeader tag="OUR SPECIALTY" lines={["Crafted for creators,", "built for impact."]} />
          ...
          return (
            <m.button
              ...
              className={`text-left p-7 rounded-card aspect-square flex flex-col justify-between transition-colors duration-300 ${
                isActive ? "bg-accent text-ink" : "bg-panel hover:bg-panel2 text-white"
              }`}
            >
              ...
              <div>
                ...
                <p className={`text-[13px] leading-snug ${isActive ? "text-ink/70" : "text-white/55"}`}>
                  {s.desc}
                </p>
              </div>
            </m.button>
...
/* ─────────────────────── PORTFOLIO GRID ─────────────────────── */
const PROJECTS = [
  { id:1, title:"Faces of Grace",  year:"2025", tags:"Portrait, Studio", img:"https://images.unsplash.com/photo-1531123897727-8f129e1688ce?w=1200&q=80&auto=format&fit=crop", featured:false },
  { id:2, title:"Family Ties",     year:"2025", tags:"People, Moments",  img:"https://images.unsplash.com/photo-1511895426328-dc8714191300?w=1200&q=80&auto=format&fit=crop", featured:true },
  { id:3, title:"Roots & Wings",   year:"2025", tags:"Sports, Events",   img:"https://images.unsplash.com/photo-1517649763962-0c623066013b?w=1200&q=80&auto=format&fit=crop", featured:false },
  { id:4, title:"Northbound",      year:"2024", tags:"Travel, Landscape",img:"https://images.unsplash.com/photo-1464822759023-fed622ff2c3b?w=1200&q=80&auto=format&fit=crop", featured:false },
  { id:5, title:"Quiet Hours",     year:"2024", tags:"Editorial, Mood",  img:"https://images.unsplash.com/photo-1488161628813-04466f872be2?w=1200&q=80&auto=format&fit=crop", featured:true },
  { id:6, title:"Concrete Days",   year:"2024", tags:"Street, Urban",    img:"https://images.unsplash.com/photo-1502635385003-ee1e6a1a742d?w=1200&q=80&auto=format&fit=crop", featured:false },
];
    ...
    <div className="flex items-start justify-between gap-4">
      <div>
        <div className="flex items-center gap-2 mb-1">
          <span className={`w-2.5 h-2.5 rounded-full ${p.featured ? "bg-accent" : "bg-white/70"}`}></span>
          <h3 className={`text-[20px] md:text-[22px] font-semibold tracking-tight ${p.featured ? "text-accent" : "text-white"}`}>{p.title}</h3>
        </div>
        <div className="text-[13px] text-white/55 pl-[18px]">{p.year} — {p.tags}</div>
      </div>
      <a href="#" className="shrink-0 inline-flex items-center justify-center w-10 h-10 rounded-full border border-line text-white/80 hover:bg-white hover:text-ink transition-colors">
...
const Portfolio = () => (
  <section data-screen-label="04 Portfolio" className="px-6 md:px-12 py-24 md:py-32">
    <Reveal>
      <div className="flex items-end justify-between flex-wrap gap-6 mb-12">
        <div>
          <TagLabel>SELECTED WORK</TagLabel>
          <h2 className="display text-4xl md:text-6xl mt-3" style={{textWrap:"balance"}}>
            Recent projects.
          </h2>
        </div>
...
const ProcessRow = ({ step, open, onToggle }) => (
  ...
  >
    <button onClick={onToggle} className="w-full flex items-center justify-between gap-6 px-6 md:px-8 py-6 text-left">
      <div className="flex items-center gap-5 md:gap-7 min-w-0">
        ...
        <span className="text-2xl md:text-3xl font-semibold tracking-tight truncate">{step.title}</span>
      </div>
      <span className="shrink-0 inline-flex items-center justify-center w-12 h-12 rounded-full bg-panel2 text-white">
        {open ? <IconMinus className="w-5 h-5"/> : <IconPlus className="w-5 h-5"/>}
      </span>
    </button>
    <AP initial={false}>
      {open && (
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6 px-6 md:px-8 pb-8">
            ...
            <div className="md:col-span-7">
              <RoundedImage src={step.img} alt={step.title} ratio="aspect-[16/9]" />
            </div>
          </div>
        </m.div>
      )}
...
const Process = () => {
  ...
  return (
    <section data-screen-label="05 Process" className="px-6 md:px-12 py-24 md:py-32">
...
const PriceCard = ({ plan, index }) => (
  ...
  >
    <div className="p-8 md:p-10">
      <h3 className="text-2xl md:text-3xl font-semibold tracking-tight">{plan.name}</h3>
      <p className={`mt-3 text-[14px] leading-snug ${plan.highlighted ? "text-ink/70" : "text-white/55"} max-w-[260px]`}>{plan.blurb}</p>
      <div className="mt-10 flex items-baseline gap-2">
        <span className="display text-5xl md:text-6xl">${plan.price}</span>
        <span className={`text-[14px] ${plan.highlighted ? "text-ink/60" : "text-white/50"}`}>{plan.unit}</span>
      </div>
      <PillButton variant={plan.highlighted ? "dark" : "accent"} className="mt-8 w-full">
        Get in Touch
      </PillButton>
    </div>
    <div className={`p-8 md:p-10 ${plan.highlighted ? "bg-ink text-white" : "bg-panel2"}`}>
...
const Pricing = () => (
  <section data-screen-label="06 Pricing" className="px-6 md:px-12 py-24 md:py-32">
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16 mb-14 items-end">
      <Reveal className="lg:col-span-5">
        <TagLabel>PRICING PLAN</TagLabel>
        <h2 className="display text-4xl md:text-5xl lg:text-6xl mt-4" style={{textWrap:"balance"}}>
          Choose a plan that fits your vision.
        </h2>
      </Reveal>
      <Reveal delay={0.1} className="lg:col-span-7 lg:pl-10 text-white/65 text-[15px] leading-relaxed max-w-[520px]">
        Transparent pricing for every kind of shoot. Need something custom?
        Tell me what you have in mind and I&apos;ll put a proposal together within
        48 hours.
      </Reveal>
    </div>
    <div className="space-y-6">
      {PLANS.map((p, i) => <PriceCard key={p.id} plan={p} index={i} />)}
    </div>
  </section>
);
...
/* ─────────────────────── TESTIMONIALS ─────────────────────── */
const REVIEWS = [
  { name:"Cody Fisher",      role:"Founder of BlueAI",  quote:"I'm still in awe of how stunning my photos turned out. Lili truly knows how to make you feel comfortable in front of the camera.", avatar:"https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=200&q=80&auto=format&fit=crop" },
  { name:"Savannah Nguyen",  role:"Founder of Ayobisa", quote:"From the moment we walked into Lili's studio, we knew we were in good hands. The whole shoot felt effortless.", avatar:"https://images.unsplash.com/photo-1438761681033-6461ffad8d80?w=200&q=80&auto=format&fit=crop" },
  { name:"Wade Warren",      role:"Founder of Lagoon",  quote:"Booked Lili for our graduation portraits — completely blown away. She directed every pose and made sure we looked our best.", avatar:"https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=200&q=80&auto=format&fit=crop" },
  { name:"Floyd Miles",      role:"Founder of Ocean",   quote:"Lili made our maternity shoot feel so special. The photos are quiet and tender — exactly what we hoped for.", avatar:"https://images.unsplash.com/photo-1544005313-94ddf0286df2?w=200&q=80&auto=format&fit=crop" },
  { name:"Jenny Wilson",     role:"Creative Director",  quote:"Our brand needed a photographer who could move fast without losing taste. Lili was that and more.", avatar:"https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=200&q=80&auto=format&fit=crop" },
];
...
const Testimonials = () => {
  ...
  return (
    <section data-screen-label="07 Testimonials" className="py-24 md:py-32 overflow-hidden">
      <SectionHeader tag="TESTIMONIAL" lines={["What our clients", "are saying."]} />
      <Reveal delay={0.1}>
        <div ref={trackRef} className="mt-14 flex gap-5 overflow-x-auto no-scrollbar px-6 md:px-12 pb-4 snap-x snap-mandatory">
          {REVIEWS.map((r, i) => (
      ...
      <div className="px-6 md:px-12 mt-8 flex items-center gap-3">
        <button
          ...
          aria-label="Previous"
        >
          <svg viewBox="0 0 24 24" className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"><path d="M15 18l-6-6 6-6"/></svg>
        </button>
        <button
          onClick={() => trackRef.current?.scrollBy({ left: 400, behavior: "smooth" })}
          className="w-12 h-12 rounded-full border border-line hover:bg-white hover:text-ink transition-colors flex items-center justify-center"
          aria-label="Next"
        >
          <svg viewBox="0 0 24 24" className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"><path d="M9 18l6-6-6-6"/></svg>
        </button>
      </div>
    </section>
  );
...
/* ─────────────────────── CTA + FOOTER ─────────────────────── */
const Footer = () => (
  <footer data-screen-label="08 Footer" className="relative px-6 md:px-12 pt-24 md:pt-32 pb-10 overflow-hidden">
    {/* big CTA */}
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
      <Reveal className="lg:col-span-7">
        <h2 className="display text-3xl md:text-5xl lg:text-[56px] tracking-tight" style={{textWrap:"balance"}}>
          <span className="text-white">Capturing brilliance in every flash</span>
          <span className="text-white/40"> — where moments turn timeless and stories shine bright.</span>
        </h2>
      </Reveal>
    ...
    <div className="mt-20 grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-6 items-start">
      <div className="md:col-span-3 space-y-2">
        ...
        {["About","Portfolio","Journal","Contact","Pricing"].map((n,i) => (
          <a key={i} href="#" className="block text-[15px] text-white/80 hover:text-accent uppercase tracking-wide">
            {n}
          </a>
        ))}
      </div>
      ...
      <div className="md:col-span-3 space-y-2 md:text-right">
        ...
        {["Portfolio Details","Journal","License","Changelog","404"].map((n,i) => (
          <a key={i} href="#" className="block text-[15px] text-white/80 hover:text-accent uppercase tracking-wide">
            {n}
          </a>
        ))}
    ...
    <div className="mt-16 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 pt-8 border-t border-line">
      ...
      <div className="flex gap-3">
        {["Instagram","Are.na","Twitter","YouTube"].map((s) => (
          <a key={s} href="#" className="px-5 py-2.5 rounded-full bg-panel text-white/80 text-[13px] hover:bg-white hover:text-ink transition-colors">
            {s}
          </a>
        ))}
      </div>
...
/* expose */
Object.assign(window, { Hero, Marquee, About, Specialty, Portfolio, Process, Pricing, Testimonials, Footer });
